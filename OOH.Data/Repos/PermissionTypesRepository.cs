...
{
    public class PermissionTypesRepository : OOHContext, IBaseRepository<PermisosMunicipalesTipos>
    {
        private readonly ILogHelper _log;

        public PermissionTypesRepository(IWebUserHelper userHelper, ILogHelper log) : base(userHelper)
        {
            _log = log;
        }
        ...
        public async Task<IEnumerable<PermisosMunicipalesTipos>> Select(string where = "")
        {
            return await SelectData<PermisosMunicipalesTipos>("SELECT * FROM PermisosMunicipalesTipos " + where);
        }
    }
}
